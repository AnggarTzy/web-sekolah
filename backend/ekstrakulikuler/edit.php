            ...
            <div class="p-6">
                ...
                <form method="POST" enctype="multipart/form-data" class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="nama" value="<?= htmlspecialchars($ekskul['nama']) ?>" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition outline-none">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="pembina" value="<?= htmlspecialchars($ekskul['pembina']) ?>" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Jadwal</label>
                        <input type="text" name="jadwal" value="<?= htmlspecialchars($ekskul['jadwal']) ?>" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi</label>
                        <textarea name="deskripsi" rows="5" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition outline-none"><?= htmlspecialchars($ekskul['deskripsi']) ?></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Gambar Saat Ini</label>
                        <?php if ($ekskul['gambar']) : ?>
                            <div class="mb-3"><img src="../../uploads/<?= $ekskul['gambar'] ?>" class="w-48 h-32 object-cover rounded-xl border border-gray-200"></div>
                        <?php else : ?>
