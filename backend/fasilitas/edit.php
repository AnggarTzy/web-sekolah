    ...
    <main class="max-w-4xl mx-auto p-6 sm:p-8">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-100">
                <h1 class="font-headline font-extrabold text-2xl text-primary">Edit Fasilitas</h1>
            </div>
            <div class="p-6">
                <?php if (isset($error)) : ?>
                ...
                <form method="POST" enctype="multipart/form-data" class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="nama" value="<?= htmlspecialchars($fasilitas['nama']) ?>" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition outline-none">
                    </div>
                    <div>
                        ...
                        <select name="kategori" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition outline-none">
                            <option value="akademik" <?= $fasilitas['kategori'] == 'akademik' ? 'selected' : '' ?>>Akademik</option>
                            <option value="olahraga" <?= $fasilitas['kategori'] == 'olahraga' ? 'selected' : '' ?>>Olahraga</option>
                            <option value="ibadah" <?= $fasilitas['kategori'] == 'ibadah' ? 'selected' : '' ?>>Ibadah</option>
                            <option value="umum" <?= $fasilitas['kategori'] == 'umum' ? 'selected' : '' ?>>Umum</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Lokasi</label>
                        <input type="text" name="lokasi" value="<?= htmlspecialchars($fasilitas['lokasi']) ?>" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Kapasitas</label>
                        <input type="text" name="kapasitas" value="<?= htmlspecialchars($fasilitas['kapasitas']) ?>" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi</label>
                        <textarea name="deskripsi" rows="5" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition outline-none"><?= htmlspecialchars($fasilitas['deskripsi']) ?></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Gambar Saat Ini</label>
                        <?php if ($fasilitas['gambar']) : ?>
                            <div class="mb-3"><img src="../../uploads/<?= $fasilitas['gambar'] ?>" class="w-48 h-32 object-cover rounded-xl border border-gray-200"></div>
                        <?php else : ?>
